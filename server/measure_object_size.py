...
def size_measuring(xmi_, ymi_, xma_, yma_, frame):
    ...
    if len(corners) == 0:
        print("WARNING: Show full aruco marker pleasee!!!!!!!!")
        return img, -1, -100
    ...
    if (xmi >= minx and xmi <= maxx and ymi >= miny and ymi <= maxy) or (
            xma >= minx and xma <= maxx and ymi >= miny and ymi <= maxy) or (
            xmi >= minx and xmi <= maxx and yma >= miny and yma <= maxy) or (
            xma >= minx and xma <= maxx and yma >= miny and yma <= maxy):
                print("WARNING: Fault positive!!!")
                return img, -2, -100
